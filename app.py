...
# SQLite path - /app always exists in Railway container
SQLITE_DB = os.getenv('DB_PATH', '/app/aml_crm.db')
...
def execute(conn, sql, params=None):
    """Execute SQL, converting ? to %s for postgres"""
    if is_pg():
        sql = sql.replace('?', '%s')
...
def fetchone(conn, sql, params=None):
    ...
    if row is None:
        return None
    return dict(row) if is_pg() else row
...
def _setup_pg(conn):
    ...
    cur.execute('SELECT id FROM users WHERE email=%s', ('admin@zewer.ae',))
    if not cur.fetchone():
    ...
    # Seed dropdowns
    cur.execute('SELECT COUNT(*) FROM dropdowns')
    if cur.fetchone()[0] == 0:
        _seed_dropdowns_pg(cur)
...
def _setup_sqlite(conn):
    ...
    if not conn.execute('SELECT id FROM dropdowns LIMIT 1').fetchone():
        _seed_dropdowns_sqlite(conn)

def _seed_dropdowns_pg(cur):
    dd = _get_dropdown_data()
    for field, items in dd.items():
        for item in items:
            val, desc = (item if isinstance(item, tuple) else (item, None))
            try:
                cur.execute('INSERT INTO dropdowns (field_name,value,description,is_active) VALUES (%s,%s,%s,1) ON CONFLICT DO NOTHING',
                    (field, val, desc))
            except: pass

def _seed_dropdowns_sqlite(conn):
    dd = _get_dropdown_data()
    for field, items in dd.items():
        for item in items:
            val, desc = (item if isinstance(item, tuple) else (item, None))
            try: conn.execute('INSERT OR IGNORE INTO dropdowns (field_name,value,description,is_active) VALUES (?,?,?,1)', (field,val,desc))
            except: pass
...
def days_left(date_str):
    ...
    try:
        d = str(date_str)[:10]
        return (datetime.strptime(d, '%Y-%m-%d').date() - datetime.now().date()).days
    except: return None
...
def get_dropdowns():
    conn = get_db()
    rows = fetchall(conn, "SELECT field_name, value FROM dropdowns WHERE is_active=1 ORDER BY field_name, value")
    conn.close()
    dd = {}
    for r in rows:
        fn = r['field_name'] if is_pg() else r['field_name']
        v = r['value']
        dd.setdefault(fn, []).append(v)
    return dd
...
def login():
    if request.method == 'POST':
        d = request.get_json()
        conn = get_db()
        u = fetchone(conn, 'SELECT * FROM users WHERE email=?', (d.get('email'),))
        conn.close()
...
def dashboard():
    ...
    def cnt(sql, p=None): return (fetchone(conn, sql, p) or {}).get('count', 0) or row_val(fetchone(conn, sql, p))

    total = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies'))
    active = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE ac_status=?',('Active',)))
    expired_tl = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE trade_license_expiry<?',(today,)))
    expiring_30_tl = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE trade_license_expiry BETWEEN ? AND ?',(today,today+timedelta(days=30))))
    expiring_90_tl = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE trade_license_expiry BETWEEN ? AND ?',(today,today+timedelta(days=90))))
    expired_ap = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE address_proof_expiry<?',(today,)))
    expiring_30_ap = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE address_proof_expiry BETWEEN ? AND ?',(today,today+timedelta(days=30))))
    expired_pass = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM ubos WHERE passport_expiry<?',(today,)))
    expiring_30_pass = row_val(fetchone(conn,'SELECT COUNT(*) as c FROM ubos WHERE passport_expiry BETWEEN ? AND ?',(today,today+timedelta(days=30))))
    risk_data = fetchall(conn,'SELECT risk_status, COUNT(*) as c FROM companies GROUP BY risk_status')
    risk_breakdown = {r['risk_status']: r['c'] for r in risk_data}
    doc_data = fetchall(conn,'SELECT doc_status, COUNT(*) as c FROM companies GROUP BY doc_status')
    doc_breakdown = {r['doc_status']: r['c'] for r in doc_data}
    urgent = fetchall(conn,'''SELECT id,ac_code,client_name,trade_license_expiry,address_proof_expiry,
        ...
        (today,today+timedelta(days=90),today,today+timedelta(days=90)))
    open_tasks = row_val(fetchone(conn,"SELECT COUNT(*) as c FROM tasks WHERE status NOT IN ('done')"))
    overdue_tasks = row_val(fetchone(conn,"SELECT COUNT(*) as c FROM tasks WHERE status NOT IN ('done','pending_close') AND due_date<?", (today,)))
    due_today = row_val(fetchone(conn,"SELECT COUNT(*) as c FROM tasks WHERE status NOT IN ('done') AND due_date=?", (today,)))
    pending_close = row_val(fetchone(conn,"SELECT COUNT(*) as c FROM tasks WHERE status='pending_close'"))
    urgent_tasks = fetchall(conn,'''SELECT t.*, u.name as assigned_name FROM tasks t
...
def companies():
    ...
    rows = fetchall(conn, q, p if p else None)
    dd = get_dropdowns()
    conn.close()
    cl = []
    for c in rows:
...
def company_new():
    conn = get_db()
    staff_users = fetchall(conn,'SELECT id,name FROM users WHERE is_active=1 ORDER BY name')
    conn.close()
...
def company_detail(id):
    conn = get_db()
    company = fetchone(conn,'SELECT * FROM companies WHERE id=?',(id,))
    if not company: conn.close(); return redirect(url_for('companies'))
    ubos = fetchall(conn,'SELECT * FROM ubos WHERE company_id=? ORDER BY share_percentage DESC',(id,))
    conn.close()
    ...
    for u in ubos:
        ...
        ubo_list.append(dict(
            id=u['id'],position=u['position'],share_percentage=u['share_percentage'],
            person_name=u['person_name'],nationality=u['nationality'],
            residential_status=u['residential_status'],passport_no=u['passport_no'],
            passport_expiry=str(u['passport_expiry']) if u['passport_expiry'] else None,
            p_days=pd,p_status=expiry_status(pd),emirates_id=u['emirates_id'],
            emirates_id_expiry=str(u['emirates_id_expiry']) if u['emirates_id_expiry'] else None,
            e_days=ed,e_status=expiry_status(ed),doc_status=u['doc_status'],verified_by=u['verified_by']))
...
def _save_ubos(conn, company_id, ubos):
    execute(conn,'DELETE FROM ubos WHERE company_id=?',(company_id,))
    for u in ubos:
        if not u.get('person_name'): continue
        execute(conn,'''INSERT INTO ubos (company_id,position,share_percentage,person_name,nationality,
...
def api_add_company():
    ...
    try:
        conn = get_db()
        execute(conn,'''INSERT INTO companies (ac_code,client_name,ac_opening_date,ac_status,active_till_year,
...
def api_edit_company(id):
    ...
    try:
        conn = get_db()
        execute(conn,'''UPDATE companies SET client_name=?,ac_opening_date=?,ac_status=?,active_till_year=?,
...
def api_delete_company(id):
    try:
        conn = get_db()
        execute(conn,'DELETE FROM companies WHERE id=?',(id,))
        conn.commit(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def alerts():
    ...
    tl = fetchall(conn,'SELECT id,ac_code,client_name,mobile,whatsapp_number,trade_license_expiry,risk_status,region FROM companies WHERE trade_license_expiry>=? ORDER BY trade_license_expiry',(cutoff,))
    ap = fetchall(conn,'SELECT id,ac_code,client_name,mobile,whatsapp_number,address_proof_expiry,address_proof_type,risk_status FROM companies WHERE address_proof_expiry>=? ORDER BY address_proof_expiry',(cutoff,))
    ubos = fetchall(conn,'''SELECT u.id,u.person_name,u.passport_no,u.passport_expiry,u.emirates_id,u.emirates_id_expiry,
...
def reports():
    ...
    total = row_val(fetchone(conn,f'SELECT COUNT(*) as c FROM companies WHERE {where}', params or None))
    def q(sql): return fetchall(conn, sql, params or None)
    result = render_template('reports.html',
        ...
        expired_tl=row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE trade_license_expiry<?',(today,))),
        expiring_30=row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE trade_license_expiry BETWEEN ? AND ?',(today,today+timedelta(days=30)))),
        expiring_90=row_val(fetchone(conn,'SELECT COUNT(*) as c FROM companies WHERE trade_license_expiry BETWEEN ? AND ?',(today,today+timedelta(days=90)))))
    conn.close()
    return result
...
def settings():
    conn = get_db()
    users = fetchall(conn,'SELECT id,email,name,role,is_active,contact_number FROM users ORDER BY role,name')
    dds = fetchall(conn,'SELECT * FROM dropdowns WHERE is_active=1 ORDER BY field_name,value')
    conn.close()
    groups = {}
...
def api_add_user():
    ...
    try:
        conn = get_db()
        execute(conn,'INSERT INTO users (email,password_hash,name,role,contact_number,is_active) VALUES (?,?,?,?,?,1)',
...
def api_edit_user(id):
    ...
    try:
        conn = get_db()
        if d.get('password'):
            execute(conn,'UPDATE users SET name=?,email=?,role=?,contact_number=?,password_hash=? WHERE id=?',
                (d.get('name'),d.get('email'),d.get('role'),d.get('contact_number'),generate_password_hash(d.get('password')),id))
        else:
            execute(conn,'UPDATE users SET name=?,email=?,role=?,contact_number=? WHERE id=?',
...
def api_toggle_user(id):
    try:
        conn = get_db()
        cur = fetchone(conn,'SELECT is_active FROM users WHERE id=?',(id,))
        if cur: execute(conn,'UPDATE users SET is_active=? WHERE id=?',(0 if cur['is_active'] else 1,id))
        conn.commit(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_delete_user(id):
    try:
        conn = get_db()
        execute(conn,'DELETE FROM users WHERE id=?',(id,))
        conn.commit(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_add_dropdown():
    ...
    try:
        conn = get_db()
        execute(conn,'INSERT INTO dropdowns (field_name,value,is_active) VALUES (?,?,1)',(d.get('field_name'),d.get('value')))
        conn.commit(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_delete_dropdown(id):
    try:
        conn = get_db()
        execute(conn,'DELETE FROM dropdowns WHERE id=?',(id,))
        conn.commit(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def tasks():
    ...
    rows = fetchall(conn, base+(' WHERE t.assigned_to=?' if role=='staff' else '')+order, (uid,) if role=='staff' else None)
    all_users = fetchall(conn,'SELECT id,name,role,mobile FROM users WHERE is_active=1 ORDER BY name')
    all_companies = fetchall(conn,'SELECT id,ac_code,client_name FROM companies ORDER BY client_name')
    task_templates = fetchall(conn,"SELECT value,description FROM dropdowns WHERE field_name='TASK TEMPLATE' AND is_active=1 ORDER BY value")
    conn.close()
    today = datetime.now().date()
    tlist = []
    for t in rows:
...
def api_add_task():
    ...
    try:
        conn = get_db()
        execute(conn,'INSERT INTO tasks (title,description,assigned_to,created_by,company_id,priority,due_date,status) VALUES (?,?,?,?,?,?,?,?)',
...
def api_edit_task(id):
    ...
    try:
        conn = get_db()
        execute(conn,'UPDATE tasks SET title=?,description=?,assigned_to=?,company_id=?,priority=?,due_date=?,updated_at=CURRENT_TIMESTAMP WHERE id=?',
...
def api_task_status(id):
    ...
    try:
        conn = get_db()
        execute(conn,'UPDATE tasks SET status=?,updated_at=CURRENT_TIMESTAMP WHERE id=?',(d.get('status'),id))
        conn.commit(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_delete_task(id):
    try:
        conn = get_db()
        execute(conn,'DELETE FROM tasks WHERE id=?',(id,))
        conn.commit(); conn.close()
        return jsonify({'success': True})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_get_documents(company_id):
    conn = get_db()
    docs = fetchall(conn,'''SELECT d.*,u.name as uploader_name FROM documents d
        LEFT JOIN users u ON d.uploaded_by=u.id WHERE d.company_id=? ORDER BY d.created_at DESC''',(company_id,))
    conn.close()
...
def api_upload_document(company_id):
    ...
    try:
        ...
        conn = get_db()
        execute(conn,'INSERT INTO documents (company_id,doc_type,file_name,file_url,public_id,uploaded_by,notes) VALUES (?,?,?,?,?,?,?)',
...
def api_delete_document(doc_id):
    try:
        conn = get_db()
        doc = fetchone(conn,'SELECT public_id FROM documents WHERE id=?',(doc_id,))
...
def export_template():
    ...
    ws.append(headers)
    ws.append(['TJ5003','SAMPLE COMPANY LLC','2020-01-01','Active','Legal entity','MainLand','N/A','Supplier',
               'United Arab Emirates','Dubai','Unit 101, Gold Souq, Dubai','+97142258019','+971506594165',
               '+971506594165','info@sample.com','Ahmed Ali','+971501234567','Jaseel','Ejari','2025-12-31',
               '534230','Dubai Economy & Tourism','Limited Liability Company(LLC)','2019-06-01','2025-12-31',
               '100003063300003','Yes','Yes',2,'Yes','Yes','Yes','Yes','Completed','Medium',
...
def export_companies():
    ...
    rows = fetchall(conn, q)
    conn.close()
    fname = f'companies_{scope}_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    if fmt == 'xlsx' and OPENPYXL_AVAILABLE:
        wb = openpyxl.Workbook(); ws = wb.active
        if rows: ws.append(list(rows[0].keys())); [ws.append(list(r.values())) for r in rows]
        out = io.BytesIO(); wb.save(out); out.seek(0)
        return send_file(out, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            as_attachment=True, download_name=fname+'.xlsx')
    out = io.StringIO(); w = csv.writer(out)
    if rows: w.writerow(rows[0].keys()); [w.writerow(r.values()) for r in rows]
    out.seek(0)
...
def export_report():
    ...
    today = datetime.now().date()
    if rtype == 'expiry':
        rows = fetchall(conn,'SELECT ac_code,client_name,trade_license_no,trade_license_expiry,address_proof_expiry,risk_status,region,account_manager FROM companies ORDER BY trade_license_expiry')
    elif rtype == 'kyc':
        rows = fetchall(conn,'SELECT ac_code,client_name,kyc_status,doc_status,risk_status,verified_by,verified_date FROM companies ORDER BY kyc_status')
    elif rtype == 'risk':
        rows = fetchall(conn,'SELECT ac_code,client_name,risk_status,doc_status,kyc_status,region,account_manager FROM companies ORDER BY risk_status')
    else:
        rows = fetchall(conn,'SELECT * FROM companies ORDER BY client_name')
    conn.close()
    fname = f'report_{rtype}_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    if fmt == 'xlsx' and OPENPYXL_AVAILABLE:
        wb = openpyxl.Workbook(); ws = wb.active; ws.title = rtype.upper()
        if rows: ws.append(list(rows[0].keys())); [ws.append([str(v) if v is not None else '' for v in r.values()]) for r in rows]
        out = io.BytesIO(); wb.save(out); out.seek(0)
        return send_file(out, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
            as_attachment=True, download_name=fname+'.xlsx')
    out = io.StringIO(); w = csv.writer(out)
...
def api_import_companies():
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            if fetchone(conn,'SELECT id FROM companies WHERE ac_code=?',(ac,)):
                skipped += 1; continue
            try:
                execute(conn,'''INSERT INTO companies (ac_code,client_name,ac_status,risk_status,doc_status,
